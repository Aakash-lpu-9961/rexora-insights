        ...
        <div className="flex-1 flex items-center gap-3 max-w-3xl">
          ...
          <div className="relative">
            ...
            >
              ...
              />
              <span className="hidden sm:inline">{activeModule.name}</span>
              <ChevronDown className="h-4 w-4 text-muted-foreground" />
            </button>
            {moduleOpen && (
              <div className="absolute right-0 mt-2 w-64 rounded-xl border border-border bg-popover shadow-[var(--shadow-floating)] p-1.5 animate-scale-in origin-top-right z-50">
                ...
                {modules.map((m) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-sm hover:bg-accent transition-colors ${
                      m.id === activeModule.id ? "bg-accent text-accent-foreground" : "text-foreground"
                    }`}
                  >
